...
<head>
    ...
    <title>Data Alumni - Kepala Sekolah - SMA PGRI 1 Subang</title>
...
<div class="main-content">
    <div class="top-bar">
        <div class="page-title"><h2>Data Alumni</h2><p>Tracking alumni SMA PGRI 1 Subang</p></div>
    </div>
    <?php if(!empty($statistik)): ?>
    <div class="stat-mini">
        <?php $warna = ['BEKERJA'=>'#10b981','KULIAH'=>'#6366f1','WIRAUSAHA'=>'#f59e0b','BELUM'=>'#94a3b8']; ?>
        <?php foreach($statistik as $st): ?>
        <div class="stat-mini-item"><div class="num" style="color:<?= $warna[$st['status_aktivitas']]??'#64748b' ?>"><?= $st['total'] ?></div><div class="lbl"><?= $st['status_aktivitas'] ?></div></div>
        <?php endforeach; ?>
    </div>
    <?php endif; ?>
    <div class="card-panel">
        <div class="table-responsive">
            <table class="table table-hover">
                <thead><tr><th>NISN</th><th>Nama</th><th>Tahun Lulus</th><th>Status</th><th>Instansi</th></tr></thead>
                <tbody>
                    <?php if(!empty($alumni)): ?>
                        ...
                        <tr>
                            ...
                            <td><?= $a['nama_alumni'] ?></td>
                            <td><?= $a['tahun_lulus'] ?></td>
                            <td><span class="badge-status" style="background:<?= $a['status_aktivitas']=='BEKERJA'?'#f0fdf4':($a['status_aktivitas']=='KULIAH'?'#eef2ff':($a['status_aktivitas']=='WIRAUSAHA'?'#fffbeb':'#f1f5f9'))?>;color:<?= $a['status_aktivitas']=='BEKERJA'?'#10b981':($a['status_aktivitas']=='KULIAH'?'#6366f1':($a['status_aktivitas']=='WIRAUSAHA'?'#f59e0b':'#64748b'))?>"><?= $a['status_aktivitas'] ?></span></td>
                            <td><?= $a['nama_instansi'] ?: '-' ?></td>
                        </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr><td colspan="5" class="text-center text-muted py-4">Belum ada data alumni.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
