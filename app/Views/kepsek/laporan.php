...
<head>
    ...
    <title>Laporan - Kepala Sekolah - SMA PGRI 1 Subang</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @media print { .sidebar, .top-bar .btn { display: none; } .main-content { margin-left: 0; } }
    </style>
</head>
<body>
<div class="sidebar">
    ...
    <a href="<?= base_url('kepsek/laporan') ?>" class="nav-item active"><i class="fa-solid fa-file-chart-bar"></i><span>Laporan</span></a>
...
<div class="main-content">
    <div class="top-bar">
        <div class="page-title"><h2>Laporan & Statistik</h2></div>
        <button onclick="window.print()" class="btn btn-dark rounded-pill px-4 py-2"><i class="fa-solid fa-print me-1"></i> Cetak</button>
    </div>
    <div class="summary-grid">
        <div class="summary-card" style="background:#eef2ff;"><div class="num" style="color:var(--primary);"><?= $total_siswa ?></div><div class="lbl" style="color:var(--dark);">Total Siswa</div></div>
        <div class="summary-card" style="background:#f0fdf4;"><div class="num" style="color:var(--secondary);"><?= $total_alumni ?></div><div class="lbl" style="color:var(--dark);">Total Alumni</div></div>
        <div class="summary-card" style="background:#fffbeb;"><div class="num" style="color:#f59e0b;"><?= $total_tracer ?></div><div class="lbl" style="color:var(--dark);">Respon Tracer</div></div>
    </div>
    <div class="row g-4">
        <div class="col-md-6"><div class="card-panel"><h5 style="font-weight:700;margin-bottom:20px;">Statistik Kelulusan</h5><table class="table"><thead><tr><th>Tahun</th><th>Total</th><th>Lulus</th><th>%</th></tr></thead><tbody><?php if(!empty($statistik_kelulusan)): foreach($statistik_kelulusan as $s): ?><tr><td><?=$s['tahun_lulus']?></td><td><?=$s['total']?></td><td><?=$s['lulus']?></td><td><?=$s['total']>0?round(($s['lulus']/$s['total'])*100,1):0?>%</td></tr><?php endforeach; else: ?><tr><td colspan="4" class="text-center text-muted">Belum ada data.</td></tr><?php endif; ?></tbody></table></div></div>
        <div class="col-md-6"><div class="card-panel"><h5 style="font-weight:700;margin-bottom:20px;">Statistik Alumni</h5><table class="table"><thead><tr><th>Status</th><th>Jumlah</th></tr></thead><tbody><?php if(!empty($statistik_alumni)): foreach($statistik_alumni as $sa): ?><tr><td><?=$sa['status_aktivitas']?></td><td><?=$sa['total']?></td></tr><?php endforeach; else: ?><tr><td colspan="2" class="text-center text-muted">Belum ada data.</td></tr><?php endif; ?></tbody></table></div></div>
        <div class="col-md-12"><div class="card-panel"><h5 style="font-weight:700;margin-bottom:20px;">Statistik Tracer Study</h5><table class="table"><thead><tr><th>Status Setelah Lulus</th><th>Jumlah</th></tr></thead><tbody><?php if(!empty($statistik_tracer)): foreach($statistik_tracer as $st): ?><tr><td><?=str_replace('_',' ',$st['status_setelah_lulus'])?></td><td><?=$st['total']?></td></tr><?php endforeach; else: ?><tr><td colspan="2" class="text-center text-muted">Belum ada data.</td></tr><?php endif; ?></tbody></table></div></div>
    </div>
</div>
