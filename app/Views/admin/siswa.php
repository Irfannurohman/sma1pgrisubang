...
<head>
    ...
    <title>Data Siswa - SMA PGRI 1 Subang</title>
...
<div class="main-content">
    <div class="top-bar">
        <div class="page-title">
            <h2>Data Siswa</h2>
            <p>Kelola seluruh data siswa SMA PGRI 1 Subang</p>
        </div>
        <a href="<?= base_url('admin/siswa/tambah') ?>" class="btn btn-dark rounded-pill px-4 py-2">
    ...
    <div class="card-panel">
        <form method="GET" action="<?= base_url('admin/siswa') ?>" class="mb-3 d-flex align-items-center gap-2">
            ...
            <a href="<?= base_url('admin/siswa') ?>" class="btn btn-sm btn-outline-secondary" style="border-radius:10px;font-size:12px;padding:6px 12px;"><i class="fa-solid fa-times"></i> Reset</a>
            <?php endif; ?>
        </form>
                ...
                <tbody>
                    <?php if(!empty($siswa)): ?>
                        ...
                        <tr>
                            ...
                            <td><?= $s['tahun_lulus'] ?></td>
                            <td>
                                <span class="badge-status" style="background:<?= $s['status_kelulusan'] == 'LULUS' ? '#f0fdf4' : '#fef2f2' ?>;color:<?= $s['status_kelulusan'] == 'LULUS' ? '#10b981' : '#ef4444' ?>;">
                                    <?= $s['status_kelulusan'] ?>
                                </span>
                            </td>
                            <td>
                                <a href="<?= base_url('admin/siswa/edit/'.$s['id']) ?>" class="btn btn-sm btn-outline-primary btn-action me-1"><i class="fa-solid fa-edit"></i></a>
                                <a href="<?= base_url('admin/siswa/hapus/'.$s['id']) ?>" class="btn btn-sm btn-outline-danger btn-action" onclick="return confirm('Hapus data siswa ini?')"><i class="fa-solid fa-trash"></i></a>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
